...
def split_Text( text, file_name, verbose = True ):
    ...
    if all_sentences > max_sentences:
       ...
       for i, small_text in enumerate( text_spans ):
           ...
           newText['_text_split_origin'] = spans[i]
    ...
    for i in range(1, len(sys.argv)):
        ...
        if os.path.isdir(arg):
            if not in_dir:
                ...
                out_dir = arg
        elif arg.lower() in ['--o', '-o', 'o'] and i+1<len(sys.argv):
            arg2 = sys.argv[i+1]
            if os.path.isdir(arg2):
               out_dir = arg2
               i += 1
        elif arg.lower() in ['--f', '-f', 'f'] and i+1<len(sys.argv):
            arg2 = sys.argv[i+1]
            if os.path.isfile(arg2):
               in_file = arg2
               i += 1
    if out_dir and (in_dir or in_file):
       start = timer()
       if in_file:
...
if not in_args_found:
   print('Please provide input arguments: \n <in_dir> <out_dir>  or  -f <in_file> -o <out_dir> !')
