...
#
#  [ This is a refactored version of CorpusBasedMorphDisambiguator that uses 
#    detached_layers. WORK IN PROGRESS ]
#
...
class CorpusBasedMorphDisambiguator( object ):
    """Provides corpus-based pre-disambiguation and post-disambiguation for morphological analysis. (work in progress version)
    ...
    def _postdisambiguate_detached_layers(self, collections, detached_layers, input_format_hint=None):
        """ Post-disambiguates ambiguous analyses based on lemma counts 
            obtained from the input corpus.
        ...
        assert len(in_collections) == len(in_detached_layers)
        assert [len(docs) for docs in in_collections] == [len(lyrs) for lyrs in in_detached_layers]
        #
        ...
        #
        for collection_id, docs in enumerate(in_collections):
