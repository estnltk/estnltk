...
class Ruleset:
    ...
    * A dynamic extraction rule allows to update initial values of attributes.
    ...
    def priority_map(self):
        result = {rule.pattern: (rule.group, rule.priority) for rule in self.static_rules}
        result.update({rule.pattern: (rule.group, rule.priority) for rule in self.dynamic_rules})
        return result
