        ...
        for i, token_span in enumerate(layers[ self._input_tokens_layer ]):
            ...
            if token_span.start in tokenization_hints:
                ...
                if end_token_index:
                    ...
                    spl = EnvelopingSpan(base_span=EnvelopingBaseSpan(s.base_span for s in spans), layer=layer,
                                         spans=spans)
    ...
    def _try_to_add_custom_abbreviation(self, raw_text: str, layers, token_id: int, compound_tokens_lists: list,
        ...
        if custom_abbreviation_found:
            ...
            if next_token == '.':
                ...
                normalized = None
            spl = EnvelopingSpan(base_span=EnvelopingBaseSpan(s.base_span for s in spans), layer=output_layer,
                                 spans=spans)
