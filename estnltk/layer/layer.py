...
class Layer:
    ...
    def resolve_attribute(self, item):
        ...
        if item not in attribute_map:
            raise AttributeError(item)

        target_layer = self.text_object[attribute_map[item]]
        if len(target_layer) == 0 or len(self) == 0:
            return AttributeList([], item)
        result = [self.text_object[attribute_map[item]].get(span.base_span)[item] for span in self]
