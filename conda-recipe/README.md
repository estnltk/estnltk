...
3. Create and set up a conda environment for the build, for instance:
	...
	* If building with the latest `conda-build` fails, you may try installing the last successfully used version of `conda-build`. In creating the last EstNLTK beta version, we used `conda-build=3.15.1` for Python 3.5 (for both x86 and x64), `conda-build=3.20.0` for Python 3.6 x64 and `conda-build=3.18.11` for Python 3.6 x86;
