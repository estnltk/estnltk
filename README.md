...
_Trobleshooting Java_: in the current release of Google Colab (2026-08-06), executing Java-based components `TimexTagger` or `ClauseSegmenter` results in `JSONDecodeError` error messages, such as:
```
json.decoder.JSONDecodeError: ("Expecting ',' delimiter: line 1 column 7 (char 6)", "in the 'ClauseSegmenter'")
```
These are cause by warnings emitted by the JVM:
```
[0.001s][warning][os,container] Cgroup memory controller path at '/sys/fs/cgroup' seems to have moved to '/../../jupyter-children', detected limits won't be accurate
[0.001s][warning][os,container] Cgroup cpu controller path at '/sys/fs/cgroup' seems to have moved to '/../../jupyter-children', detected limits won't be accurate
```
A current workaround for this problem is to modify the EstNLTK source at 
[`/usr/local/lib/python3.13/dist-packages/estnltk/java/javaprocess.py:102`](https://github.com/estnltk/estnltk/blob/713fb4b8bbafca9a39c4534862ec6aecc2c98472/estnltk/estnltk/java/javaprocess.py#L102) by adding the flag `-Xlog:os+container=off` :

```
        self._process = subprocess.Popen(['java', '-Xlog:os+container=off', '-jar', self._runnable_jar] + self._java_args, 
```
This will switch off the warnings and allow to capture the JSON output generated by the Java components.
